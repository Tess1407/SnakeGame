...
internal class Program
{
    ...
    private static void Setup()
    {
        ...
        for (var i = 0; i < _size.Height; i++)
        {
            ...
            {
                $"+{new string('-', _size.Width - 2)}+".Write(0, i);               // game border
            }
            else
            {
    ...
    private static void Draw()
    {
        ...
        {
            new string(' ', _size.Width - 2).Write(1, i, ConsoleColor.Blue, ConsoleColor.Black);
        }

        _head.Write("O", ConsoleColor.Yellow, ConsoleColor.Black);
        _tail.Write("o");                                                               // how the snake looks like 
        _fruit.Write("♥", ConsoleColor.Red, ConsoleColor.Black);                        // how the fruit looks like
    ...
    private static void Input()
    {
        ...
        if ((key == ConsoleKey.UpArrow) || (key == ConsoleKey.W)) { _direction = Direction.Up; }        // snake's movement with keyboard
        else if ((key == ConsoleKey.DownArrow) || (key == ConsoleKey.S)) { _direction = Direction.Down; }
        else if ((key == ConsoleKey.LeftArrow) || (key == ConsoleKey.A)) { _direction = Direction.Left; }
        else if ((key == ConsoleKey.RightArrow) || (key == ConsoleKey.D)) { _direction = Direction.Right; }
        else if ((key == ConsoleKey.Escape) || (key == ConsoleKey.Enter)) { _direction = Direction.Stop; }
    }
    ...
    private static void Logic()
    {
        ...
        if (_tail.Contains(_head))                              // logic od the game, when the snake bites himself, game is over
        {
            _gameOver = true;
            _direction = Direction.Stop;
            return;
        ...
        if (_head == _fruit) { _fruit = RandomPoint(); }        // drawing snake's tail
    ...
    {
        var x = _random.Next(1, _size.Width - 1);       // fruit for snake
